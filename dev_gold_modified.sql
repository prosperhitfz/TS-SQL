...
SELECT T2.name , count(*) FROM singer_in_concert AS T1 JOIN singer AS T2 ON T1.singer_id = T2.singer_id GROUP BY T2.singer_id	concert_singer
 SELECT DISTINCT singer.Name FROM concert JOIN singer_in_concert ON concert.concert_ID = singer_in_concert.concert_ID JOIN singer ON singer_in_concert.Singer_ID = singer.Singer_ID WHERE concert.Year = '2014'; 	concert_singer
 SELECT DISTINCT singer.Name FROM concert JOIN singer_in_concert ON concert.concert_ID = singer_in_concert.concert_ID JOIN singer ON singer_in_concert.Singer_ID = singer.Singer_ID WHERE concert.Year = '2014'; 	concert_singer
...
SELECT stuid FROM student EXCEPT SELECT T1.stuid FROM student AS T1 JOIN has_pet AS T2 ON T1.stuid = T2.stuid JOIN pets AS T3 ON T3.petid = T2.petid WHERE T3.pettype = 'cat'	pets_1
 SELECT   s.Fname, s.Age FROM   Student s WHERE   s.StuID IN (  SELECT   hp.StuID  FROM   Has_Pet hp  INNER JOIN Pets p ON hp.PetID = p.PetID  WHERE   p.PetType = 'dog'  )  AND s.StuID NOT IN (  SELECT   hp.StuID  FROM   Has_Pet hp  INNER JOIN Pets p ON hp.PetID = p.PetID  WHERE   p.PetType = 'cat'  ); 	pets_1
 SELECT   distinct s.Fname FROM   Student s WHERE   s.StuID IN (  SELECT   hp.StuID  FROM   Has_Pet hp  INNER JOIN Pets p ON hp.PetID = p.PetID  WHERE   p.PetType = 'dog'  )  AND s.StuID NOT IN (  SELECT   hp.StuID  FROM   Has_Pet hp  INNER JOIN Pets p ON hp.PetID = p.PetID  WHERE   p.PetType = 'cat'  ); 	pets_1
...
SELECT T1.FullName , T1.Id , count(*) FROM CAR_MAKERS AS T1 JOIN MODEL_LIST AS T2 ON T1.Id = T2.Maker GROUP BY T1.Id;	car_1
 SELECT ml.Model FROM cars_data cd JOIN car_names cn ON cd.Id = cn.MakeId JOIN model_list ml ON cn.Model = ml.Model WHERE CAST(NULLIF(cd.Horsepower, '') AS INTEGER) = (  SELECT MIN(CAST(NULLIF(Horsepower, '') AS INTEGER))  FROM cars_data  WHERE Horsepower GLOB '*[0-9]*'  ) LIMIT 1; 	car_1
 SELECT ml.Model FROM cars_data cd JOIN car_names cn ON cd.Id = cn.MakeId JOIN model_list ml ON cn.Model = ml.Model WHERE CAST(NULLIF(cd.Horsepower, '') AS INTEGER) = (  SELECT MIN(CAST(NULLIF(Horsepower, '') AS INTEGER))  FROM cars_data  WHERE Horsepower GLOB '*[0-9]*'  ) LIMIT 1; 	car_1
...
SELECT count(*) FROM MODEL_LIST AS T1 JOIN CAR_MAKERS AS T2 ON T1.Maker = T2.Id JOIN COUNTRIES AS T3 ON T2.Country = T3.CountryId WHERE T3.CountryName = 'usa';	car_1
 SELECT AVG(CAST(MPG AS REAL)) AS Average_MPG FROM cars_data WHERE Cylinders = 4 AND MPG GLOB '[0-9]*'; 	car_1
 SELECT AVG(CAST(MPG AS REAL)) AS Average_MPG FROM cars_data WHERE Cylinders = 4 AND MPG GLOB '[0-9]*'; 	car_1
...
SELECT T1.CountryName FROM COUNTRIES AS T1 JOIN CONTINENTS AS T2 ON T1.Continent = T2.ContId JOIN CAR_MAKERS AS T3 ON T1.CountryId = T3.Country WHERE T2.Continent = 'europe' GROUP BY T1.CountryName HAVING count(*) >= 3;	car_1
 SELECT   cn.Make,   MAX(CAST(cd.Horsepower AS INTEGER)) AS MaxHorsepower FROM   cars_data cd  JOIN   car_names cn ON cd.Id = cn.MakeId  WHERE   cd.Cylinders = 3 GROUP BY   cn.Make ORDER BY   MaxHorsepower DESC 	car_1
 SELECT   cn.Make,   MAX(CAST(cd.Horsepower AS INTEGER)) AS MaxHorsepower FROM   cars_data cd  JOIN   car_names cn ON cd.Id = cn.MakeId  WHERE   cd.Cylinders = 3 GROUP BY   cn.Make ORDER BY   MaxHorsepower DESC 	car_1
 SELECT ml.Model FROM cars_data cd  JOIN car_names cn ON cd.Id = cn.MakeId  JOIN model_list ml ON cn.Model = ml.Model  ORDER BY CAST(cd.MPG AS REAL) DESC LIMIT 1; 	car_1
 SELECT ml.Model FROM cars_data cd  JOIN car_names cn ON cd.Id = cn.MakeId  JOIN model_list ml ON cn.Model = ml.Model  ORDER BY CAST(cd.MPG AS REAL) DESC LIMIT 1; 	car_1
 SELECT AVG(CAST(Horsepower AS REAL)) AS AverageHorsepower  FROM cars_data  WHERE Year < 1980  AND Horsepower GLOB '*[0-9]*'; 	car_1
 SELECT AVG(CAST(Horsepower AS REAL)) AS AverageHorsepower  FROM cars_data  WHERE Year < 1980  AND Horsepower GLOB '*[0-9]*'; 	car_1
...
SELECT T1.FullName , T1.Id FROM CAR_MAKERS AS T1 JOIN MODEL_LIST AS T2 ON T1.Id = T2.Maker GROUP BY T1.Id HAVING count(*) > 3;	car_1
 SELECT DISTINCT ml.Model FROM model_list ml JOIN car_makers cm ON ml.Maker = cm.Id WHERE cm.FullName = 'General Motors'  UNION  SELECT DISTINCT cn.Model FROM car_names cn JOIN cars_data cd ON cn.MakeId = cd.Id WHERE cd.Weight > 3500; 	car_1
 SELECT DISTINCT ml.Model FROM model_list ml JOIN car_makers cm ON ml.Maker = cm.Id WHERE cm.FullName = 'General Motors'  UNION  SELECT DISTINCT cn.Model FROM car_names cn JOIN cars_data cd ON cn.MakeId = cd.Id WHERE cd.Weight > 3500; 	car_1
...
SELECT COUNT(*) FROM CARS_DATA WHERE Accelerate > ( SELECT Accelerate FROM CARS_DATA ORDER BY CAST(Horsepower AS INTEGER) DESC LIMIT 1 );	car_1
 SELECT COUNT(*) AS NumberOfCountries FROM (  SELECT countries.CountryId  FROM countries  JOIN car_makers ON countries.CountryId = car_makers.Country  GROUP BY countries.CountryId  HAVING COUNT(car_makers.Id) > 2 ) AS countries_with_more_than_two_makers; 	car_1
 SELECT COUNT(*) AS NumberOfCountries FROM (  SELECT countries.CountryId  FROM countries  JOIN car_makers ON countries.CountryId = car_makers.Country  GROUP BY countries.CountryId  HAVING COUNT(car_makers.Id) > 2 ) AS countries_with_more_than_two_makers; 	car_1
SELECT COUNT(*) FROM CARS_DATA WHERE Cylinders > 6;	car_1
SELECT COUNT(*) FROM CARS_DATA WHERE Cylinders > 6;	car_1
 WITH filtered_cars AS (  SELECT cn.Model, CAST(cd.Horsepower AS REAL) AS Horsepower  FROM cars_data cd  JOIN car_names cn ON cd.Id = cn.MakeId  WHERE cd.Cylinders = 4 AND cd.Horsepower GLOB '[0-9]*' ), max_horsepower AS (  SELECT MAX(Horsepower) AS max_hp  FROM filtered_cars ) SELECT Model FROM filtered_cars WHERE Horsepower = (SELECT max_hp FROM max_horsepower) LIMIT 1; 	car_1
 WITH filtered_cars AS (  SELECT cn.Model, CAST(cd.Horsepower AS REAL) AS Horsepower  FROM cars_data cd  JOIN car_names cn ON cd.Id = cn.MakeId  WHERE cd.Cylinders = 4 AND cd.Horsepower GLOB '[0-9]*' ), max_horsepower AS (  SELECT MAX(Horsepower) AS max_hp  FROM filtered_cars ) SELECT Model FROM filtered_cars WHERE Horsepower = (SELECT max_hp FROM max_horsepower) LIMIT 1; 	car_1
SELECT cn.MakeId, cn.Make FROM cars_data cd JOIN car_names cn ON cd.Id = cn.MakeId WHERE CAST(cd.Horsepower AS INTEGER) > (  SELECT MIN(CAST(Horsepower AS INTEGER)) FROM cars_data WHERE Horsepower GLOB '[0-9]*'  ) AND cd.Cylinders <= 3;	car_1
SELECT cn.MakeId, cn.Make FROM cars_data cd JOIN car_names cn ON cd.Id = cn.MakeId WHERE CAST(cd.Horsepower AS INTEGER) > (  SELECT MIN(CAST(Horsepower AS INTEGER)) FROM cars_data WHERE Horsepower GLOB '[0-9]*'  ) AND cd.Cylinders <= 3;	car_1
 SELECT MAX(CAST(MPG AS REAL)) AS Max_MPG FROM cars_data WHERE Cylinders = 8 OR Year < 1980; 	car_1
 SELECT MAX(CAST(MPG AS REAL)) AS Max_MPG FROM cars_data WHERE Cylinders = 8 OR Year < 1980; 	car_1
...
SELECT count(*) FROM AIRLINES WHERE Country = "USA"	flight_2
 SELECT City, Country FROM airports  WHERE AirportName LIKE '%Alton%'; 	flight_2
 SELECT City, Country FROM airports  WHERE AirportName LIKE '%Alton%'; 	flight_2
SELECT AirportName FROM AIRPORTS WHERE AirportCode = "AKO"	flight_2
SELECT AirportName FROM AIRPORTS WHERE AirportCode = "AKO"	flight_2
 SELECT AirportName FROM airports  WHERE TRIM(City) = 'Aberdeen'; 	flight_2
 SELECT AirportName FROM airports  WHERE TRIM(City) = 'Aberdeen'; 	flight_2
 SELECT COUNT(*) AS Flights_Departing_From_APG FROM flights WHERE TRIM(SourceAirport) = 'APG'; 	flight_2
 SELECT COUNT(*) AS Flights_Departing_From_APG FROM flights WHERE TRIM(SourceAirport) = 'APG'; 	flight_2
 SELECT COUNT(*) AS Flights_to_ATO FROM flights  WHERE TRIM(DestAirport) = 'ATO'; 	flight_2
 SELECT COUNT(*) AS Flights_to_ATO FROM flights  WHERE TRIM(DestAirport) = 'ATO'; 	flight_2
...
SELECT count(*) FROM FLIGHTS AS T1 JOIN AIRPORTS AS T2 ON TRIM(T1.DestAirport) = TRIM(T2.AirportCode) JOIN AIRPORTS AS T3 ON TRIM(T1.SourceAirport) = TRIM(T3.AirportCode) WHERE TRIM(T2.City) = 'Ashley' AND TRIM(T3.City )= 'Aberdeen'	flight_2
SELECT count(*) FROM FLIGHTS AS T1 JOIN AIRLINES AS T2 ON T1.Airline = T2.uid WHERE T2.Airline = "JetBlue Airways"	flight_2
SELECT count(*) FROM FLIGHTS AS T1 JOIN AIRLINES AS T2 ON T1.Airline = T2.uid WHERE T2.Airline = "JetBlue Airways"	flight_2
 SELECT COUNT(*)  FROM flights  JOIN airlines ON flights.Airline = airlines.uid  WHERE airlines.Airline = 'United Airlines'  AND TRIM(flights.DestAirport) = 'ASY'; 	flight_2
 SELECT COUNT(*)  FROM flights  JOIN airlines ON flights.Airline = airlines.uid  WHERE airlines.Airline = 'United Airlines'  AND TRIM(flights.DestAirport) = 'ASY'; 	flight_2
 SELECT COUNT(*) as NumberOfFlights FROM flights  WHERE flights.Airline = 1 AND TRIM(flights.SourceAirport) = 'AHD'; 	flight_2
 SELECT COUNT(*) as NumberOfFlights FROM flights  WHERE flights.Airline = 1 AND TRIM(flights.SourceAirport) = 'AHD'; 	flight_2
...
SELECT T1.City FROM AIRPORTS AS T1 JOIN FLIGHTS AS T2 ON T1.AirportCode = T2.DestAirport GROUP BY T1.City ORDER BY count(*) DESC LIMIT 1	flight_2
 SELECT a.City  FROM flights f  JOIN airports a ON TRIM(f.SourceAirport) = TRIM(a.AirportCode)  GROUP BY a.City  ORDER BY COUNT(f.FlightNo) DESC LIMIT 1; 	flight_2
 SELECT a.City  FROM flights f  JOIN airports a ON TRIM(f.SourceAirport) = TRIM(a.AirportCode)  GROUP BY a.City  ORDER BY COUNT(f.FlightNo) DESC LIMIT 1; 	flight_2
...
SELECT T1.AirportCode FROM AIRPORTS AS T1 JOIN FLIGHTS AS T2 ON T1.AirportCode = T2.DestAirport OR T1.AirportCode = T2.SourceAirport GROUP BY T1.AirportCode ORDER BY count(*) DESC LIMIT 1	flight_2
SELECT T1.AirportCode FROM AIRPORTS AS T1 JOIN FLIGHTS AS T2 ON TRIM(T1.AirportCode) = TRIM(T2.DestAirport) OR TRIM(T1.AirportCode) = TRIM(T2.SourceAirport) GROUP BY T1.AirportCode ORDER BY count(*)  LIMIT 1;	flight_2
SELECT T1.AirportCode FROM AIRPORTS AS T1 JOIN FLIGHTS AS T2 ON TRIM(T1.AirportCode) = TRIM(T2.DestAirport) OR TRIM(T1.AirportCode) = TRIM(T2.SourceAirport) GROUP BY T1.AirportCode ORDER BY count(*)  LIMIT 1;	flight_2
...
SELECT T1.Abbreviation , T1.Country FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline GROUP BY T1.Airline ORDER BY count(*) LIMIT 1	flight_2
 SELECT DISTINCT airlines.Airline  FROM flights  JOIN airlines ON flights.Airline = airlines.uid  WHERE TRIM(flights.SourceAirport) = 'AHD'; 	flight_2
 SELECT DISTINCT airlines.Airline  FROM flights  JOIN airlines ON flights.Airline = airlines.uid  WHERE TRIM(flights.SourceAirport) = 'AHD'; 	flight_2
 SELECT DISTINCT a.Airline FROM flights f JOIN airlines a ON f.Airline = a.uid WHERE TRIM(f.DestAirport) = 'AHD'; 	flight_2
 SELECT DISTINCT a.Airline FROM flights f JOIN airlines a ON f.Airline = a.uid WHERE TRIM(f.DestAirport) = 'AHD'; 	flight_2
SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'APG' INTERSECT SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'CVO'	flight_2
SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'APG' INTERSECT SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'CVO'	flight_2
SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'CVO' EXCEPT SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'APG'	flight_2
SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'CVO' EXCEPT SELECT T1.Airline FROM AIRLINES AS T1 JOIN FLIGHTS AS T2 ON T1.uid = T2.Airline WHERE TRIM(T2.SourceAirport) = 'APG'	flight_2
...
SELECT T1.id , T1.name FROM battle AS T1 JOIN ship AS T2 ON T1.id = T2.lost_in_battle JOIN death AS T3 ON T2.id = T3.caused_by_ship_id GROUP BY T1.id HAVING sum(T3.killed) > 10	battle_death
 SELECT ship.id, ship.name FROM ship JOIN death ON ship.id = death.caused_by_ship_id GROUP BY ship.id, ship.name ORDER BY SUM(death.injured) DESC LIMIT 1; 	battle_death
...
SELECT department_description FROM Departments WHERE department_name LIKE '%computer%'	student_transcripts_tracking
 SELECT   s.student_id,  s.first_name,  s.middle_name,  s.last_name FROM   Students s JOIN   Student_Enrolment se ON s.student_id = se.student_id GROUP BY   s.student_id, se.semester_id HAVING   COUNT(DISTINCT se.degree_program_id) > 1; 	student_transcripts_tracking
 SELECT   s.student_id,  s.first_name,  s.middle_name,  s.last_name FROM   Students s JOIN   Student_Enrolment se ON s.student_id = se.student_id GROUP BY   s.student_id, se.semester_id HAVING   COUNT(DISTINCT se.degree_program_id) > 1; 	student_transcripts_tracking
...
SELECT T1.degree_program_id , T1.degree_summary_name FROM Degree_Programs AS T1 JOIN Student_Enrolment AS T2 ON T1.degree_program_id = T2.degree_program_id GROUP BY T1.degree_program_id ORDER BY count(*) DESC LIMIT 1	student_transcripts_tracking
 WITH enrollment_counts AS (  SELECT se.student_id, COUNT(se.student_enrolment_id) AS enrollment_count   FROM Student_Enrolment se   GROUP BY se.student_id ), max_enrollments AS (  SELECT MAX(enrollment_count) AS max_enrollment_count   FROM enrollment_counts ), top_student AS (  SELECT student_id   FROM enrollment_counts   WHERE enrollment_count = (SELECT max_enrollment_count FROM max_enrollments)  ORDER BY student_id  LIMIT 1 ) SELECT s.student_id AS id, s.first_name, s.middle_name, s.last_name, ec.enrollment_count FROM enrollment_counts ec JOIN top_student ts ON ec.student_id = ts.student_id JOIN Students s ON ec.student_id = s.student_id;	student_transcripts_tracking
 WITH enrollment_counts AS (  SELECT se.student_id, COUNT(se.student_enrolment_id) AS enrollment_count   FROM Student_Enrolment se   GROUP BY se.student_id ), max_enrollments AS (  SELECT MAX(enrollment_count) AS max_enrollment_count   FROM enrollment_counts ), top_student AS (  SELECT student_id   FROM enrollment_counts   WHERE enrollment_count = (SELECT max_enrollment_count FROM max_enrollments)  ORDER BY student_id  LIMIT 1 ) SELECT s.student_id AS id, s.first_name, s.middle_name, s.last_name, ec.enrollment_count FROM enrollment_counts ec JOIN top_student ts ON ec.student_id = ts.student_id JOIN Students s ON ec.student_id = s.student_id;	student_transcripts_tracking
...
SELECT first_name FROM Students WHERE current_address_id != permanent_address_id	student_transcripts_tracking
 WITH AddressFrequencies AS (  SELECT current_address_id AS address_id, COUNT(*) AS student_count   FROM Students   GROUP BY current_address_id )  SELECT a.address_id, a.line_1, a.line_2, a.line_3 FROM Addresses a  JOIN AddressFrequencies af ON a.address_id = af.address_id  WHERE af.student_count = (  SELECT MAX(student_count) FROM AddressFrequencies ) 	student_transcripts_tracking
...
SELECT count(*) FROM Cartoon WHERE Written_by = "Joseph Kuhr";	tvshow
 SELECT Title, Directed_by  FROM Cartoon  ORDER BY   CASE   WHEN LENGTH(Original_air_date) BETWEEN 13 AND 15 THEN   printf('%04d-%02d-%02d',   CAST(SUBSTR(Original_air_date, -4) AS INTEGER),   CASE WHEN SUBSTR(Original_air_date, 1, 3) = 'Jan' THEN 1  WHEN SUBSTR(Original_air_date, 1, 3) = 'Feb' THEN 2  WHEN SUBSTR(Original_air_date, 1, 3) = 'Mar' THEN 3  WHEN SUBSTR(Original_air_date, 1, 3) = 'Apr' THEN 4  WHEN SUBSTR(Original_air_date, 1, 3) = 'May' THEN 5  WHEN SUBSTR(Original_air_date, 1, 3) = 'Jun' THEN 6  WHEN SUBSTR(Original_air_date, 1, 3) = 'Jul' THEN 7  WHEN SUBSTR(Original_air_date, 1, 3) = 'Aug' THEN 8  WHEN SUBSTR(Original_air_date, 1, 3) = 'Sep' THEN 9  WHEN SUBSTR(Original_air_date, 1, 3) = 'Oct' THEN 10  WHEN SUBSTR(Original_air_date, 1, 3) = 'Nov' THEN 11  WHEN SUBSTR(Original_air_date, 1, 3) = 'Dec' THEN 12  END,  CAST(SUBSTR(Original_air_date, 5, LENGTH(SUBSTR(Original_air_date, 5, INSTR(Original_air_date, ',')-5))) AS INTEGER)  )  END; 	tvshow
 SELECT Title, Directed_by  FROM Cartoon  ORDER BY   CASE   WHEN LENGTH(Original_air_date) BETWEEN 13 AND 15 THEN   printf('%04d-%02d-%02d',   CAST(SUBSTR(Original_air_date, -4) AS INTEGER),   CASE WHEN SUBSTR(Original_air_date, 1, 3) = 'Jan' THEN 1  WHEN SUBSTR(Original_air_date, 1, 3) = 'Feb' THEN 2  WHEN SUBSTR(Original_air_date, 1, 3) = 'Mar' THEN 3  WHEN SUBSTR(Original_air_date, 1, 3) = 'Apr' THEN 4  WHEN SUBSTR(Original_air_date, 1, 3) = 'May' THEN 5  WHEN SUBSTR(Original_air_date, 1, 3) = 'Jun' THEN 6  WHEN SUBSTR(Original_air_date, 1, 3) = 'Jul' THEN 7  WHEN SUBSTR(Original_air_date, 1, 3) = 'Aug' THEN 8  WHEN SUBSTR(Original_air_date, 1, 3) = 'Sep' THEN 9  WHEN SUBSTR(Original_air_date, 1, 3) = 'Oct' THEN 10  WHEN SUBSTR(Original_air_date, 1, 3) = 'Nov' THEN 11  WHEN SUBSTR(Original_air_date, 1, 3) = 'Dec' THEN 12  END,  CAST(SUBSTR(Original_air_date, 5, LENGTH(SUBSTR(Original_air_date, 5, INSTR(Original_air_date, ',')-5))) AS INTEGER)  )  END; 	tvshow
...
SELECT Pixel_aspect_ratio_PAR , country FROM tv_channel WHERE LANGUAGE != 'English'	tvshow
 SELECT t.id  FROM TV_Channel t WHERE t.Country IN (  SELECT Country  FROM TV_Channel  GROUP BY Country  HAVING COUNT(id) > 2 ); 	tvshow
...
SELECT T3.area_code FROM contestants AS T1 JOIN votes AS T2 ON T1.contestant_number = T2.contestant_number JOIN area_code_state AS T3 ON T2.state = T3.state WHERE T1.contestant_name = 'Tabatha Gehling' INTERSECT SELECT T3.area_code FROM contestants AS T1 JOIN votes AS T2 ON T1.contestant_number = T2.contestant_number JOIN area_code_state AS T3 ON T2.state = T3.state WHERE T1.contestant_name = 'Kelly Clauss'	voter_1
 SELECT contestant_name FROM CONTESTANTS WHERE contestant_name LIKE '%Al%' AND contestant_name NOT LIKE '%Walser%'; 	voter_1
SELECT Name FROM country WHERE IndepYear > 1950	world_1
SELECT Name FROM country WHERE IndepYear > 1950	world_1
SELECT count(*) FROM country WHERE GovernmentForm = "Republic"	world_1
 SELECT COUNT(*) AS RepublicCountriesCount FROM country WHERE GovernmentForm LIKE '%Republic%'; 	world_1
...
SELECT Name FROM country WHERE Continent = "Asia" AND population > (SELECT max(population) FROM country WHERE Continent = "Africa")	world_1
 SELECT Name FROM country  WHERE Continent = 'Asia' AND Population > (  SELECT MAX(Population) FROM country  WHERE Continent = 'Africa' ); 	world_1
 SELECT DISTINCT c.Code  FROM country c  WHERE c.Code NOT IN (  SELECT cl.CountryCode  FROM countrylanguage cl  WHERE cl.Language = 'English' ); 	world_1
 SELECT DISTINCT c.Code  FROM country c  WHERE c.Code NOT IN (  SELECT cl.CountryCode  FROM countrylanguage cl  WHERE cl.Language = 'English' ); 	world_1
...
SELECT Code FROM country WHERE GovernmentForm != "Republic" EXCEPT SELECT CountryCode FROM countrylanguage WHERE LANGUAGE = "English"	world_1
 SELECT DISTINCT c.Code  FROM country c WHERE c.Code NOT IN (  SELECT cl.CountryCode   FROM countrylanguage cl   WHERE cl.Language = 'English' )  AND c.Code NOT IN (  SELECT c2.Code  FROM country c2  WHERE c2.GovernmentForm LIKE '%Republic%' ); 	world_1
...
SELECT LANGUAGE FROM countrylanguage GROUP BY LANGUAGE ORDER BY count(*) DESC LIMIT 1	world_1
 WITH RankedLanguages AS (  SELECT  CountryCode,  Language,  Percentage,  ROW_NUMBER() OVER (PARTITION BY CountryCode ORDER BY Percentage DESC, Language) AS rn  FROM countrylanguage ) SELECT CountryCode, Language FROM RankedLanguages WHERE rn = 1; 	world_1
 WITH RankedLanguages AS (  SELECT  CountryCode,  Language,  Percentage,  ROW_NUMBER() OVER (PARTITION BY CountryCode ORDER BY Percentage DESC, Language) AS rn  FROM countrylanguage ) SELECT CountryCode, Language FROM RankedLanguages WHERE rn = 1; 	world_1
 WITH SpanishLargest AS (  SELECT   cl.CountryCode  FROM   countrylanguage cl  INNER JOIN (  SELECT   CountryCode,   MAX(Percentage) AS MaxPercentage  FROM   countrylanguage  GROUP BY   CountryCode  ) AS max_lang ON cl.CountryCode = max_lang.CountryCode AND cl.Percentage = max_lang.MaxPercentage  WHERE   cl.Language = 'Spanish' ) SELECT   COUNT(DISTINCT CountryCode) AS NumberOfCountries FROM   SpanishLargest; 	world_1
 WITH SpanishLargest AS (  SELECT   cl.CountryCode  FROM   countrylanguage cl  INNER JOIN (  SELECT   CountryCode,   MAX(Percentage) AS MaxPercentage  FROM   countrylanguage  GROUP BY   CountryCode  ) AS max_lang ON cl.CountryCode = max_lang.CountryCode AND cl.Percentage = max_lang.MaxPercentage  WHERE   cl.Language = 'Spanish' ) SELECT   COUNT(DISTINCT CountryCode) AS NumberOfCountries FROM   SpanishLargest; 	world_1
 WITH max_percentage AS (  SELECT MAX(Percentage) as max_percent  FROM countrylanguage  WHERE Language = 'Spanish' ) SELECT CountryCode FROM countrylanguage WHERE Language = 'Spanish' AND Percentage = (SELECT max_percent FROM max_percentage); 	world_1
 SELECT cl.CountryCode FROM countrylanguage cl JOIN (  SELECT CountryCode, MAX(Percentage) as MaxPercentage  FROM countrylanguage  GROUP BY CountryCode ) max_cl ON cl.CountryCode = max_cl.CountryCode WHERE cl.Language = 'Spanish' AND cl.Percentage = max_cl.MaxPercentage; 	world_1
...
SELECT T2.name,  count(friend_id) FROM Friend AS T1 RIGHT JOIN Highschooler AS T2 ON T1.student_id  =  T2.id GROUP BY T2.id	network_1
SELECT T2.name,  count(friend_id) FROM Friend AS T1 RIGHT JOIN Highschooler AS T2 ON T1.student_id  =  T2.id GROUP BY T2.id	network_1
...
SELECT T1.owner_id ,  T1.last_name  FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id  =  T2.owner_id  JOIN Treatments AS T3 ON T2.dog_id  =  T3.dog_id GROUP BY T1.owner_id  ORDER BY sum(cost_of_treatment) DESC  LIMIT 1	dog_kennels
...
SELECT T1.owner_id , T1.zip_code FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id JOIN Treatments AS T3 ON T2.dog_id = T3.dog_id GROUP BY T1.owner_id ORDER BY sum(T3.cost_of_treatment) DESC LIMIT 1	dog_kennels
SELECT T1.professional_id ,  T1.cell_number  FROM Professionals AS T1 JOIN Treatments AS T2 ON T1.professional_id  =  T2.professional_id  GROUP BY T1.professional_id  HAVING count(DISTINCT treatment_type_code)  >=  2	dog_kennels
SELECT T1.professional_id ,  T1.cell_number  FROM Professionals AS T1 JOIN Treatments AS T2 ON T1.professional_id  =  T2.professional_id  GROUP BY T1.professional_id  HAVING count(DISTINCT treatment_type_code)  >=  2	dog_kennels
SELECT DISTINCT T1.first_name ,  T1.last_name FROM Professionals AS T1 JOIN Treatments AS T2 ON T1.professional_id = T2.professional_id WHERE cost_of_treatment  <  ( SELECT avg(cost_of_treatment) FROM Treatments )	dog_kennels
SELECT DISTINCT T1.first_name ,  T1.last_name FROM Professionals AS T1 JOIN Treatments AS T2 ON T1.professional_id = T2.professional_id WHERE cost_of_treatment  <  ( SELECT avg(cost_of_treatment) FROM Treatments )	dog_kennels
...
SELECT DISTINCT T1.date_arrived , T1.date_departed FROM Dogs AS T1 JOIN Treatments AS T2 ON T1.dog_id = T2.dog_id	dog_kennels
SELECT T1.last_name FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T2.age = ( SELECT min(age) FROM Dogs )	dog_kennels
SELECT T1.last_name FROM Owners AS T1 JOIN Dogs AS T2 ON T1.owner_id = T2.owner_id WHERE T2.age = ( SELECT min(age) FROM Dogs )	dog_kennels
...
SELECT avg(age) FROM Dogs	dog_kennels
SELECT MAX(CAST(age AS SIGNED)) FROM Dogs;	dog_kennels
SELECT MAX(CAST(age AS SIGNED)) FROM Dogs;	dog_kennels
...
SELECT email_address , cell_number , home_phone FROM professionals	dog_kennels
SELECT DISTINCT breed_name,size_description FROM Dogs as T1 join Breeds as T2 on T1.breed_code = T2.breed_code join Sizes as T3 on T1.size_code = T3.size_code	dog_kennels
SELECT DISTINCT breed_name,size_description FROM Dogs as T1 join Breeds as T2 on T1.breed_code = T2.breed_code join Sizes as T3 on T1.size_code = T3.size_code	dog_kennels
...
SELECT Name FROM singer WHERE Singer_ID NOT IN (SELECT Singer_ID FROM song)	singer
SELECT Citizenship FROM singer WHERE Birth_Year  <  1945 union SELECT Citizenship FROM singer WHERE Birth_Year  >  1955	singer
SELECT Citizenship FROM singer WHERE Birth_Year  <  1945  union SELECT Citizenship FROM singer WHERE Birth_Year  >  1955 	singer
